...
(function(scope, undef) {
	...
	(function(name, definition) {
		...
		cookieOptions = function(o) {
			...
			return ([ (typeof o.expires === 'object' && o.expires instanceof Date ? '; expires=' + o.expires.toGMTString() : ''), ('; path=' + o.path), (typeof o.domain === 'string' ? '; domain=' + o.domain : ''),
					(o.secure === true ? '; secure' : '') ].join(''));
		},
		/**
		 *
		 * @access private
		 ...
		 */
		isNaN = (function() {
			var p = /\d/, native_isNaN = scope.isNaN;
		...
		parseCookies = (function() {
			...
			return function() {
				var c = {}, s1 = document.cookie.split(';'), q = s1.length, i, s2, n, v, vv;
				...
				for (i = 0; i < q; i += 1) {
					...
					try {
						vv = (vv === 'true') ? true : (vv === 'false') ? false : !isNaN(vv) ? parseFloat(vv) : p.test(vv) ? parseJSON(vv) : vv;
					} catch (ignore) {
					}
		...
		return {
			 ...
			 */
			test : function() {
				var r = false, n = 'test_cookies_jaaulde_js', v = 'data';
